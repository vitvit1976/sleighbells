...
public final class SampleUsers {
     ...
     * </p>
     */
    static final List<DevUser> getListUsers(){
        List<DevUser> users = new ArrayList<>();
      //  ALL_USERS.add(USER);
      //  ALL_USERS.add(ADMIN);
        for(String s: Arrays.asList(ADMIN_USERNAME, "az", "buki", "vedi", "gamma")) {
            users.add(DevUser.builder().preferredUsername(s).fullName(s.toUpperCase())
                    .userId(USER_ID).password(s).email(s +"@example.com")
                            .profileUrl(s + "=" + s.toUpperCase())
                    .roles(s.equals(ADMIN_USERNAME) || s.equals("gamma") ? AppRoles.ADMIN : AppRoles.USER).
                    build());
        }
        return users;
    }
    public static final List<DevUser> ALL_USERS = getListUsers(); //List.of(USER, ADMIN);
}
